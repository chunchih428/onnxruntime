...
static constexpr const char* QNN_HTP_SHARED = "QnnHtpShared";
...
QnnEpFactory::QnnEpFactory(const char* ep_name, ApiPtrs apis, OrtHardwareDeviceType device_type, const char* backend_type)
  ...
  // Setup HTP shared memory info (HOST_ACCESSIBLE for data transfer)
  mem_info = nullptr;
  status = ort_api.CreateMemoryInfo_V2(QNN_HTP_SHARED, OrtMemoryInfoDeviceType_NPU,
                                       QUALCOMM_VENDOR_ID, /* device_id */ 0,
                                       ...
                                       &mem_info);
  assert(status == nullptr);
  htp_shared_memory_info_ = MemoryInfoUniquePtr(mem_info, ort_api.ReleaseMemoryInfo);

  // GPU allocator OrtMemoryInfo for future use (optional)
  mem_info = nullptr;
  status = ort_api.CreateMemoryInfo_V2("QnnEP GPU", OrtMemoryInfoDeviceType_GPU,
                                       QUALCOMM_VENDOR_ID, /* device_id */ 0,
                                       OrtDeviceMemoryType_DEFAULT,
                                       /*alignment*/ 0,
                                       OrtAllocatorType::OrtDeviceAllocator,
  ...
  // GPU HOST_ACCESSIBLE memory for future use (optional)
  mem_info = nullptr;
  status = ort_api.CreateMemoryInfo_V2("QnnEP GPU pinned", OrtMemoryInfoDeviceType_GPU,
                                       QUALCOMM_VENDOR_ID, /* device_id */ 0,
                                       OrtDeviceMemoryType_HOST_ACCESSIBLE,
                                       /*alignment*/ 0,
                                       OrtAllocatorType::OrtDeviceAllocator,
  ...
  // Setup data transfer for NPU/HTP with shared memory support
  data_transfer_impl_ = std::make_unique<ExampleDataTransfer>(
      apis,
      ep_api.MemoryInfo_GetMemoryDevice(npu_memory_info_.get()),         // NPU device memory
      ep_api.MemoryInfo_GetMemoryDevice(htp_shared_memory_info_.get())   // HTP shared memory for data transfer
  );

  // Keep data transfer enabled for NPU/HTP memory operations
  // data_transfer_impl_.reset();  // commented out to enable data transfer
}
