...
// Simplified QnnSerializerConfig for ABI - commented out as not needed for basic functionality
...
struct QnnBackendManagerConfig {
  ...
  // ProfilingLevel profiling_level_etw;  // Commented out for ABI
  // ProfilingLevel profiling_level;       // Commented out for ABI
  std::string profiling_file_path;
  // ContextPriority context_priority;  // Commented out for ABI
  // std::shared_ptr<QnnSerializerConfig> qnn_serializer_config;  // Commented out for ABI
  uint32_t device_id;
  // QnnHtpDevice_Arch_t htp_arch;  // Commented out for ABI
  uint32_t soc_model;
 ...
 public:
  ...
  QnnBackendManager(const QnnBackendManagerConfig& config, PrivateConstructorTag)
      : backend_path_(config.backend_path),
        ort_api_(nullptr),
        // profiling_level_etw_(config.profiling_level_etw),  // Commented out for ABI
        // profiling_level_(config.profiling_level),  // Commented out for ABI
        profiling_file_path_(config.profiling_file_path),
        // context_priority_(config.context_priority),  // Commented out for ABI
        // qnn_serializer_config_(config.qnn_serializer_config),  // Commented out for ABI
        device_id_(config.device_id),
        // htp_arch_(config.htp_arch),  // Commented out for ABI
        soc_model_(config.soc_model),
        op_packages_(config.op_packages) {
  }

  // Method to set ORT API access
  void SetOrtApi(const OrtApi* ort_api) { ort_api_ = ort_api; }

  // Helper method to get context file path for a node (for VTCM backup buffer sharing)
  std::string GetContextFilePathForNode(const std::string& node_name) const {
    std::lock_guard<std::mutex> guard(ep_context_handle_map_mutex_);
    auto it = ep_context_handle_map_abi_.find(node_name);
    return (it != ep_context_handle_map_abi_.end()) ? it->second : std::string();
  }
  ...
  // std::unique_ptr<unsigned char[]> GetContextBinaryBuffer(uint64_t& written_buffer_size);  // Commented out for ABI

  // Status LoadCachedQnnContextFromBuffer(char* buffer, uint64_t buffer_length,
  //                                       std::string node_name,
  //                                       std::unordered_map<std::string, std::unique_ptr<qnn::QnnModel>>& qnn_models,
  //                                       int64_t max_spill_fill_size);  // Commented out for ABI

  // Initializes handles to QNN resources (device, logger, etc.).
  // NOTE: This function locks the internal `logger_recursive_mutex_`.
  OrtStatus* SetupBackend(const OrtLogger* logger, bool load_from_cached_context,
...
//
 private:
  OrtStatus* LoadBackend();

  OrtStatus* InitializeBackend();

  OrtStatus* CreateDevice();

  OrtStatus* ReleaseDevice();

  OrtStatus* ShutdownBackend();

  OrtStatus* InitializeProfiling();

  OrtStatus* ReleaseProfilehandle();

  OrtStatus* CreateContext(bool enable_htp_weight_sharing);

  OrtStatus* CreateContextVtcmBackupBufferSharingEnabled(std::unordered_map<std::string,
                                                                        std::unique_ptr<std::vector<std::string>>>& context_bin_map);

  OrtStatus* ReleaseContext();
  ...
  OrtStatus* InitializeQnnLog(const OrtLogger* logger);

  // Terminate logging in the backend
  // NOTE: This function locks the internal `logger_recursive_mutex_`.
  OrtStatus* TerminateQnnLog();
  ...
  OrtStatus* ReleaseResources();

  void* LoadLib(const char* file_name, int flags, std::string& error_msg);

  OrtStatus* LoadQnnSystemLib();

  OrtStatus* LoadQnnSerializerBackend();

  OrtStatus* LoadOpPackage();

  OrtStatus* UnloadLib(void* handle);

  // Get backend build ID
  std::string GetBackendBuildId();

  // Real implementation functions for QNN-ABI LoadBackend
  OrtStatus* GetQnnInterfaceProvider(const char* lib_path,
                                     const char* interface_provider_name,
                                     void** backend_lib_handle,
                                     void** interface_provider);
  template <class T>
  T ResolveSymbol(void* lib_handle, const char* symbol_name, std::string& error_msg);
  uint32_t GetRealBackendId(void* backend_interface_provider, const std::string& backend_path);
  uint32_t SimulateGetBackendId(const std::string& backend_path);
  void SimulateSetQnnBackendType(uint32_t backend_id);
  void SetQnnBackendType(uint32_t backend_id);
  std::string GetSimulatedProviderName(uint32_t backend_id);
//
...
//
 private:
  ...
  const OrtLogger* logger_ = nullptr;  // Changed to OrtLogger for ABI
  const OrtApi* ort_api_;  // ORT API access for QNN-ABI
  // QNN_INTERFACE_VER_TYPE qnn_interface_ = QNN_INTERFACE_VER_TYPE_INIT;  // Commented out for ABI
  // QNN_SYSTEM_INTERFACE_VER_TYPE qnn_sys_interface_ = QNN_SYSTEM_INTERFACE_VER_TYPE_INIT;  // Commented out for ABI
  void* backend_lib_handle_ = nullptr;
  void* system_lib_handle_ = nullptr;
  // Qnn_BackendHandle_t backend_handle_ = nullptr;  // Commented out for ABI
  // QnnBackend_Config_t** backend_config_ = nullptr;  // Commented out for ABI
  // Qnn_LogHandle_t log_handle_ = nullptr;  // Commented out for ABI
  // Qnn_DeviceHandle_t device_handle_ = nullptr;  // Commented out for ABI

  // Essential member variables for QNN-ABI
  std::string profiling_file_path_;
  uint32_t device_id_;
  uint32_t soc_model_;
  std::vector<OpPackage> op_packages_;
  bool backend_setup_completed_ = false;
  bool vtcm_backup_buffer_sharing_enabled_ = false;
  std::string sdk_build_version_;

  // QNN-ABI compatible context mapping for VTCM backup buffer sharing
  mutable std::mutex ep_context_handle_map_mutex_;
  std::unordered_map<std::string, std::string> ep_context_handle_map_abi_;  // Maps node names to context file paths
  ...
  // std::unordered_map<Qnn_ContextHandle_t, std::shared_ptr<QnnContextHandleRecord>> context_map_;  // Commented out for ABI

  // Map of EP Main Context Node names to Qnn_ContextHandle_t
  // std::mutex ep_context_handle_map_mutex_;  // Commented out for ABI
  // std::unordered_map<std::string, Qnn_ContextHandle_t> ep_context_handle_map_;  // Commented out for ABI

  // Vector of Qnn_ContextHandle_t. The context handles are owned by context_map_.
  // std::vector<Qnn_ContextHandle_t> contexts_;  // Commented out for ABI

  // ProfilingLevel profiling_level_etw_;  // Commented out for ABI
  // ProfilingLevel profiling_level_;  // Commented out for ABI
  // ProfilingLevel profiling_level_merge_;  // Commented out for ABI
  // const std::string profiling_file_path_;  // Already defined above
  // bool backend_initialized_ = false;  // Commented out for ABI
  // bool device_created_ = false;  // Commented out for ABI
  // bool context_created_ = false;  // Commented out for ABI
  // bool backend_setup_completed_ = false;  // Already defined above
  // bool vtcm_backup_buffer_sharing_enabled_ = false;  // Already defined above
  // NPU backend requires quantized model
  // QnnBackendType qnn_backend_type_ = QnnBackendType::CPU;  // Commented out for ABI
  // Qnn_ProfileHandle_t profile_backend_handle_ = nullptr;  // Commented out for ABI
  // ContextPriority context_priority_;  // Commented out for ABI
  // std::string sdk_build_version_ = "";  // Already defined above
// #ifdef _WIN32
//   std::set<HMODULE> mod_handles_;
// #endif
//   const std::shared_ptr<QnnSerializerConfig> qnn_serializer_config_;
//   uint32_t device_id_ = 0;  // Already defined above
//   QnnHtpDevice_Arch_t htp_arch_ = QNN_HTP_DEVICE_ARCH_NONE;  // Commented out for ABI
//   uint32_t soc_model_ = QNN_SOC_MODEL_UNKNOWN;  // Already defined above
//   const std::vector<OpPackage> op_packages_;  // Already defined above
};
