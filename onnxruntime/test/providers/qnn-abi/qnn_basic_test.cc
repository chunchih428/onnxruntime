...
#include "core/providers/qnn-abi/qnn_allocator.h"  // Used by QnnHTPBackendABITests.UseHtpSharedMemoryAllocatorForInputs
#endif
...
TEST_F(QnnHTPBackendABITests, TestAddEpUsingPublicApi) {
...
TEST(QnnEPABI, TestDisableCPUFallback_BackendNotFound) {
  {
    Ort::SessionOptions so;
    so.AddConfigEntry(kOrtSessionOptionsDisableCPUEPFallback, "1");  // Disable fallback to the CPU EP.
...
TEST(QnnEPABI, TestDisableCPUFallback_ModelNotFullySupported) {
  {
    Ort::SessionOptions so;
    so.AddConfigEntry(kOrtSessionOptionsDisableCPUEPFallback, "1");  // Disable fallback to the CPU EP.
...
TEST(QnnEPABI, TestDisableCPUFallback_TryingToRunOnQnnCPU) {
  SessionOptions so;
...
TEST(QnnEPABI, TestDisableCPUFallback_ConflictingConfig) {
  {
    Ort::SessionOptions so;
    so.AddConfigEntry(kOrtSessionOptionsDisableCPUEPFallback, "1");  // Disable fallback to the CPU EP.
...
TEST(QnnEPABI, TestInvalidSpecificationOfBothBackendTypeAndBackendPath) {
...
TEST_F(QnnHTPBackendABITests, TestConvWithExternalData) {
  Ort::SessionOptions so;
  onnxruntime::ProviderOptions options;
#if defined(_WIN32)
  options["backend_path"] = "QnnHtp.dll";
...
TEST_F(QnnHTPBackendABITests, RunConvInt4Model) {
  Ort::SessionOptions so;
...
TEST_F(QnnCPUBackendABITests, TestNHWCResizeShapeInference_scales_opset11) {
...
// the scales input. Use the QNN CPU backend.
TEST_F(QnnCPUBackendABITests, TestNHWCResizeShapeInference_scales_opset18) {
...
TEST_F(QnnCPUBackendABITests, TestNHWCResizeShapeInference_sizes_opset11) {
  RunNHWCResizeModel(ORT_MODEL_FOLDER "nhwc_resize_sizes_opset11.onnx", TestBackend::Cpu);
}

// Test shape inference of NHWC Resize operator (opset 18) that uses
// the sizes input. Use the QNN CPU backend.
TEST_F(QnnCPUBackendABITests, TestNHWCResizeShapeInference_sizes_opset18) {
...
TEST_F(QnnCPUBackendABITests, QnnSaver_OutputFiles) {
...
TEST_F(QnnCPUBackendABITests, MultithreadSessionRun) {
...
TEST_F(QnnHTPBackendABITests, MultithreadSessionRun) {
  std::unique_ptr<ModelAndBuilder> model;
  std::vector<float> input_data = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  std::vector<int64_t> shape = {1, 3, 2};
  std::vector<std::vector<int64_t>> output_shapes = {shape};
...
TEST_F(QnnHTPBackendABITests, MultithreadHtpPowerCfgSessionRunOption) {
  std::unique_ptr<ModelAndBuilder> model;
  std::vector<float> input_data = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  std::vector<int64_t> shape = {1, 3, 2};
  std::vector<std::vector<int64_t>> output_shapes = {shape};
...
TEST_F(QnnHTPBackendABITests, MultithreadDefaultHtpPowerCfgFromEpOption) {
  std::unique_ptr<ModelAndBuilder> model;
  std::vector<float> input_data = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  std::vector<int64_t> shape = {1, 3, 2};
  std::vector<std::vector<int64_t>> output_shapes = {shape};
...
TEST_F(QnnHTPBackendABITests, MultithreadHtpPowerCfgDefaultAndRunOption) {
  std::unique_ptr<ModelAndBuilder> model;
  std::vector<float> input_data = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
  std::vector<int64_t> shape = {1, 3, 2};
  std::vector<std::vector<int64_t>> output_shapes = {shape};
...
TEST_F(QnnHTPBackendABITests, TestNHWCResizeShapeInference_qdq_sizes_opset18) {
...
TEST_F(QnnHTPBackendABITests, QnnIr_OutputFiles) {
...
TEST_F(QnnHTPBackendABITests, QnnSaver_OutputFiles) {
  const std::filesystem::path qnn_saver_output_dir = "saver_output";
...
TEST_F(QnnHTPBackendABITests, HTPGraphFinalizationOptimizationModes) {
...
TEST_F(QnnHTPBackendABITests, HTPSocModels) {
...
TEST_F(QnnHTPBackendABITests, HTPArchValues) {
...
TEST_F(QnnHTPBackendABITests, QnnContextPriorityHigh) {
  RunNHWCResizeModel(ORT_MODEL_FOLDER "nhwc_resize_sizes_opset18.quant.onnx",
...
TEST_F(QnnHTPBackendABITests, ProfilingTest) {
  onnxruntime::ProviderOptions provider_options;
...
TEST_F(QnnHTPBackendABITests, CastAddHTPAccuracyTest) {
...
TEST_F(QnnHTPBackendABITests, Float32ModelWithFP16PrecisionTest) {
  ProviderOptions provider_options;
#if defined(_WIN32)
  provider_options["backend_path"] = "QnnHtp.dll";
#else
...
TEST_F(QnnHTPBackendABITests, EPRejectsDynamicShapesF32) {
...
TEST_F(QnnHTPBackendABITests, DumpJsonQNNGraph) {
...
TEST_F(QnnHTPBackendABITests, EPOffloadsGraphIOQuantDequant) {
...
#if BUILD_QNN_EP_STATIC_LIB
TEST_F(QnnHTPBackendABITests, UseHtpSharedMemoryAllocatorForInputs) {
  ProviderOptions provider_options;
#if defined(_WIN32)
  provider_options["backend_path"] = "QnnHtp.dll";
#else
...
TEST_F(QnnHTPBackendABITests, LoadingAndUnloadingOfQnnLibrary_FixSegFault) {
  const ORTCHAR_T* ort_model_path = ORT_MODEL_FOLDER "nhwc_resize_sizes_opset18.quant.onnx";
...
TEST_F(QnnHTPBackendABITests, AutoEp_PreferNpu) {
...
TEST_F(QnnHTPBackendABITests, TestMismatchedGraphInputAndTensorWrapperCount) {
  onnxruntime::ProviderOptions provider_options;
...
TEST_F(QnnIRBackendABITests, QnnIr_OutputFiles) {
  const std::filesystem::path qnn_dlc_dir = kDlcOutputDir;
...
TEST(QnnSaverBackendABITests, QnnSaver_OutputFiles) {
  const std::filesystem::path qnn_saver_output_dir = "saver_output";
