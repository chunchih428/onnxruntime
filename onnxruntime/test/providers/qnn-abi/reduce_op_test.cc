...
TEST_F(QnnCPUBackendABITests, ReduceSumOpset13_Int32) {
...
TEST_F(QnnCPUBackendABITests, ReduceSumOpset11_Int32) {
  RunReduceTest<int32_t>("ReduceSum",
                         TestInputDef<int32_t>({2, 2}, false, -10.0f, 10.0f),
                         std::vector<int64_t>{0, 1},
                         true,  // keepdims
...
// - Uses opset 13, which has "axes" as an input.
TEST_F(QnnCPUBackendABITests, ReduceSumOpset13_Float) {
...
// - The input and output data type is float.
// - Uses opset 11, which has "axes" as an attribute.
TEST_F(QnnCPUBackendABITests, ReduceSumOpset11_Float) {
  RunReduceTest<float>("ReduceSum",
                       TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
TEST_F(QnnCPUBackendABITests, ReduceProdOpset18) {
...
TEST_F(QnnCPUBackendABITests, ReduceProdOpset18_SlightlyInaccurate_WindowsLinuxX64) {
  RunReduceTest<float>("ReduceProd",
...
TEST_F(QnnCPUBackendABITests, ReduceProdOpset13) {
  RunReduceTest<float>("ReduceProd",
                       TestInputDef<float>({2, 2}, false, {-10.0f, -8.2f, 0.0f, 10.0f}),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
// - The input and output data type is float.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnCPUBackendABITests, ReduceMaxOpset18) {
...
// - The input and output data type is float.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnCPUBackendABITests, ReduceMaxOpset13) {
  RunReduceTest<float>("ReduceMax",
                       TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
// - The input and output data type is float.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnCPUBackendABITests, ReduceMinOpset18) {
...
// - The input and output data type is float.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnCPUBackendABITests, ReduceMinOpset13) {
  RunReduceTest<float>("ReduceMin",
                       TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
// - The input and output data type is float.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnCPUBackendABITests, ReduceMeanOpset18) {
...
// - The input and output data type is float.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnCPUBackendABITests, ReduceMeanOpset13) {
  RunReduceTest<float>("ReduceMean",
                       TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
// ReduceL2
//
TEST_F(QnnCPUBackendABITests, ReduceL2Opset18) {
...
TEST_F(QnnCPUBackendABITests, ReduceL2Opset13) {
  RunReduceTest<float>("ReduceL2",
                       TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                       std::vector<int64_t>{0, 1},
                       true,  // keepdims
...
// Issue fixed in 2.30
TEST_F(QnnHTPBackendABITests, ReduceSumOpset11_5D_FP16) {
...
// - Uses opset 13, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceSumU8Opset13) {
...
TEST_F(QnnHTPBackendABITests, ReduceSumU8Opset13_LastAxis) {
...
// - Uses uint8 as the quantization type.
// - Uses opset 11, which has "axes" as an attribute.
TEST_F(QnnHTPBackendABITests, ReduceSumU8Opset11) {
  RunReduceOpQDQTest<uint8_t>("ReduceSum",
                              TestInputDef<float>({2, 2}, false, {-10.0f, 3.21289f, -5.9981f, 10.0f}),
                              {0, 1},  // axes
                              true,    // keepdims
...
// - Uses opset 13, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceSumS8Opset13) {
...
TEST_F(QnnHTPBackendABITests, ReduceSumS8Opset13_NoKeepDims) {
...
TEST_F(QnnHTPBackendABITests, ReduceSumS8Opset13_Rank5) {
  RunReduceOpQDQTest<int8_t>("ReduceSum",
...
TEST_F(QnnHTPBackendABITests, ReduceSumS8Opset13_Rank6_Unsupported) {
  RunReduceOpQDQTest<int8_t>("ReduceSum",
...
TEST_F(QnnHTPBackendABITests, ReduceSumU8Opset13_Rank5_LastAxis) {
...
// - Uses uint8 as the quantization type.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceMaxU8Opset18) {
...
// - Uses uint8 as the quantization type.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnHTPBackendABITests, ReduceMaxU8Opset13) {
  RunReduceOpQDQTest<uint8_t>("ReduceMax",
                              TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                              {0, 1},  // axes
                              true,    // keepdims
...
// - Uses int8 as the quantization type.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceMaxS8Opset18) {
  std::vector<float> input_data = GetFloatDataInRange(-10.0f, 10.0f, 9);
...
// - Uses uint8 as the quantization type.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceMinU8Opset18) {
...
// - Uses uint8 as the quantization type.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnHTPBackendABITests, ReduceMinU8Opset13) {
  RunReduceOpQDQTest<uint8_t>("ReduceMin",
                              TestInputDef<float>({2, 2}, false, -10.0f, 10.0f),
                              {0, 1},  // axes
                              true,    // keepdims
...
//
// Uses int8 as the quantization type.
TEST_F(QnnHTPBackendABITests, ReduceMinS8Opset18) {
  std::vector<float> input_data = GetFloatDataInRange(-10.0f, 10.0f, 9);
...
// - Uses uint8 as the quantization type.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceMeanU8Opset18) {
...
TEST_F(QnnHTPBackendABITests, ReduceMeanU8Opset18_LastAxis) {
  const std::vector<float> input_data = {3.21289f, -5.9981f, -1.72799f, 6.27263f};
  RunReduceOpQDQTest<uint8_t>("ReduceMean",
                              TestInputDef<float>({2, 2}, false, input_data),
                              {1},   // axes
...
// - Uses uint8 as the quantization type.
// - Uses opset 13, which has "axes" as an attribute.
TEST_F(QnnHTPBackendABITests, ReduceMeanU8Opset13) {
  RunReduceOpQDQTest<uint8_t>("ReduceMean",
                              TestInputDef<float>({2, 2}, false, {-10.0f, 3.21289f, -5.9981f, 10.0f}),
                              {0, 1},  // axes
                              true,    // keepdims
...
// - Uses int8 as the quantization type.
// - Uses opset 18, which has "axes" as an input.
TEST_F(QnnHTPBackendABITests, ReduceMeanS8Opset18) {
