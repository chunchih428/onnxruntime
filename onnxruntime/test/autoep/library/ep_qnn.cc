...
/// </summary>
struct QnnMulKernel {
  QnnMulKernel(const OrtApi& ort_api, const OrtLogger& logger,
  ...
  OrtStatus* Compute(OrtKernelContext* kernel_context) {
    RETURN_IF_ERROR(ort_api.Logger_LogMessage(&logger,
                                              ...
                                              "QnnMulKernel::Compute", ORT_FILE, __LINE__, __FUNCTION__));
    ...
    RETURN_IF(num_outputs != 1, ort_api, "Expected 1 output for QnnMulKernel");
...
/// <summary>
/// QNN OrtNodeComputeInfo that represents the computation function for a compiled OrtGraph.
/// </summary>
...
QnnEp::QnnEp(QnnEpFactory& factory, const std::string& name, const Config& config, const OrtLogger& logger)
  ...
  auto status = ort_api.Logger_LogMessage(&logger_,
                                          OrtLoggingLevel::ORT_LOGGING_LEVEL_INFO,
                                          ("QnnEp has been created with name " + name_).c_str(),
...
/*static*/
OrtStatus* ORT_API_CALL QnnEp::GetCapabilityImpl(OrtEp* this_ptr, const OrtGraph* graph,
  ...
  // This QNN EP sets this to true and saves initializers during the call to OrtEp::Compile for use
  // during inference.
...
/*static*/
OrtStatus* ORT_API_CALL QnnEp::CompileImpl(_In_ OrtEp* this_ptr, _In_ const OrtGraph** graphs,
  ...
  // Associate the name of the fused node with our QnnMulKernel.
  const char* fused_node_name = nullptr;
  RETURN_IF_ERROR(ort_api.Node_GetName(fused_nodes[0], &fused_node_name));

  ep->kernels_.emplace(std::string(fused_node_name), std::make_unique<QnnMulKernel>(ep->ort_api, ep->logger_,
...
// This is an example implementation that can be used to generate an EPContext model. However, this QNN EP
// cannot currently run the EPContext model.
OrtStatus* QnnEp::CreateEpContextNodes(gsl::span<const OrtNode*> fused_nodes,
...
//
// Implementation of QnnNodeComputeInfo
//
QnnNodeComputeInfo::QnnNodeComputeInfo(QnnEp& ep) : ep(ep) {
...
OrtStatus* QnnNodeComputeInfo::CreateStateImpl(OrtNodeComputeInfo* this_ptr,
                                                   OrtNodeComputeContext* compute_context,
                                                   void** compute_state) {
  ...
  QnnMulKernel& kernel = *kernel_it->second;
  *compute_state = &kernel;
  return nullptr;
}

OrtStatus* QnnNodeComputeInfo::ComputeImpl(OrtNodeComputeInfo* this_ptr, void* compute_state,
                                               OrtKernelContext* kernel_context) {
  (void)this_ptr;
  QnnMulKernel& kernel = *reinterpret_cast<QnnMulKernel*>(compute_state);
  return kernel.Compute(kernel_context);
}

void QnnNodeComputeInfo::ReleaseStateImpl(OrtNodeComputeInfo* this_ptr, void* compute_state) {
  (void)this_ptr;
  QnnMulKernel& kernel = *reinterpret_cast<QnnMulKernel*>(compute_state);
  (void)kernel;
